//===- LowerGemmini.cpp -  Lower matrix intrinsics -----*- C++ -*-===//
//
...
static cl::opt<MatrixLayoutTy> MatrixLayout(
    "gemmini-matrix-default-layout", cl::init(MatrixLayoutTy::ColumnMajor),
...
/// LowerGemmini contains the methods used to lower matrix intrinsics.
///
...
public:
  LowerGemmini(Function &F, TargetTransformInfo &TTI,
  ...
  CallInst* gemmini_extended_mv(IRBuilder<> &Builder, bool is_mvin, Value* dram_addr, Value* spad_addr, uint64_t cols, uint64_t rows) {
    StringRef asmString;
    if (is_mvin)
      asmString = ".insn r CUSTOM_1, " STR(k_MVIN) ", 4, x0, $0, $1";
    else
      asmString = ".insn r CUSTOM_1, " STR(k_MVOUT) ", 4, x0, $0, $1";
...
PreservedAnalyses LowerGemminiPass::run(Function &F,
  ...
  LowerGemmini LMT(F, TTI, AA, DT, LI, ORE);
  if (LMT.Visit()) {
...
void LowerGemminiPass::printPipeline(
    raw_ostream &OS, function_ref<StringRef(StringRef)> MapClassName2PassName) {
  static_cast<PassInfoMixin<LowerGemminiPass> *>(this)->printPipeline(
      OS, MapClassName2PassName);
  OS << '<';
  OS << '>';
}

llvm::PassPluginLibraryInfo getLowerGemminiPassPluginInfo() {
  return {LLVM_PLUGIN_API_VERSION, "LowerGemminiPass", LLVM_VERSION_STRING,
          [](PassBuilder &PB) {
            PB.registerPipelineParsingCallback(
                [](StringRef Name, FunctionPassManager &FPM,
                   ArrayRef<PassBuilder::PipelineElement>) {
                  if (Name == "LowerGemminiPass") {
                    FPM.addPass(LowerGemminiPass());
                    return true;
                  }
                  return false;
                });
...
llvmGetPassPluginInfo() {
  return getLowerGemminiPassPluginInfo();
}
